...
class Product(db.Model):
    ...
    unit_id = db.Column(
        db.BigInteger, db.ForeignKey("units.id"), nullable=False
    )  # Foreign key referencing the units table
    max_stock = db.Column(
        db.BigInteger, nullable=False
    )  # Maximum stock quantity, cannot be null
    min_stock = db.Column(
        db.BigInteger, nullable=False
    )  # Minimum stock quantity, cannot be null
    current_stock = db.Column(
        db.BigInteger, nullable=False
    )  # Current stock quantity, cannot be null
    ...
    warehouse_move_product = db.relationship(
        "WarehouseMoveProduct", back_populates="product"
    )
    operation_logs = db.relationship(
        "OperationLog", back_populates="product", cascade="all, delete-orphan"
    )
