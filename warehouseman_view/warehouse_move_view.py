...
class WarehousemanWarehouseMoveModelView(ModelView):
    ...
    column_formatters = {
        "user.username": lambda view, context, model, name: model.user.username
        if model.user
        else "N/A",
        "order_date": lambda view, context, model, name: model.order_date.strftime(
            "%Y-%m-%d"
        )
        if model.order_date
        else "N/A",
        "implementation_date": lambda view, context, model, name: model.implementation_date.strftime(
            "%Y-%m-%d"
        )
        if model.implementation_date
        else "N/A",
    }
    form_args = {
        "order_date": {"validators": [validators.InputRequired()]},
        "implementation_date": {"validators": [validators.InputRequired()]},
    }
    ...
    def on_model_change(self, form, model, is_created):
        ...
        return super(WarehousemanWarehouseMoveModelView, self).on_model_change(
            form, model, is_created
        )

    def _on_form_prefill(self, form, id):
        return super(WarehousemanWarehouseMoveModelView, self)._on_form_prefill(
            form, id
        )
