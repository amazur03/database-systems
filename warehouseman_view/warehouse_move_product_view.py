...
class WarehousemanWarehouseMoveProductModelView(ModelView):
    ...
    column_formatters = {
        "warehouse_move.move_type": lambda view, context, model, name: model.warehouse_move.move_type
        if model.warehouse_move
        else "N/A",
        "product.name": lambda view, context, model, name: model.product.name
        if model.product
        else "N/A",
        "quantity": lambda view, context, model, name: model.quantity
        if model.quantity
        else "N/A",
    }
    form_args = {
    ...
    def on_model_change(self, form, model, is_created):
        ...
        return super(WarehousemanWarehouseMoveProductModelView, self).on_model_change(
            form, model, is_created
        )

    def _on_form_prefill(self, form, id):
        return super(WarehousemanWarehouseMoveProductModelView, self)._on_form_prefill(
            form, id
        )
